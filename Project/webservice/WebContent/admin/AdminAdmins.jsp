...
function advan_ask(){
	if(confirm("관리자를 일반회원으로 강등시키겠습니까?")){
		button.onclick();
		return true;
	}
	else{
	...
	<%
		if(!a_id.equals("root")){
			out.println("<script>alert('총 책임자가 아닙니다');</script>");
			out.println("<script> location.replace('./admin/Admin.jsp'); </script>");
		}
	%>
...
<article>
	...
	<p>관리자 관리</p>
	 <div id="searchtable-area">
		<form action="./AdminAdmins.jsp" method="post" id="searchtalbe-form">
    ...
    <table class="admintb">
        ...
        <tr>
            <th width=10%>관리자번호</th>
            <th width=30%>아이디(이메일)</th>
            <th width=20%>관리자이름</th>
            <th width=20%>닉네임</th>
            <th width=10%>권한</th>
            <th width=10%>관리</th>
        </tr>
      ...
      <form aciton="" id="setRows">
        ...
        try{
    		...
    		if(s_word==null){rs = stmt.executeQuery(
					"select * from server.admin");
					} else{rs = stmt.executeQuery(
					"select * from server.admin where admin_nick like '%"+s_word+"%'");
					}
    		int num = 1;
    		int total= 0;
    		while(rs.next()){
    		...
    		for(int i=0; i<total;i++){
    			rs.next();
    			String email = rs.getString("admin_id");
    			String password = rs.getString("admin_pw");
    			String nick = rs.getString("admin_nick");
    			String name = rs.getString("admin_name");
    			String birth = rs.getString("admin_birth");
    			String gender = rs.getString("admin_gender");
        ...
        %>
            ...
            <td width=20%><%=name %></td>
            <td width=20%><%=nick %></td>
            <td width=10%><form action="AdminDemot.jsp" method="post">
            ...
            <button class="advanbtn" onClick="advan_ask(); return false;">강등</button></form></td>
            <td width=10%><a href="AdminDelete.jsp?email=<%=email%>"><button class="deletebtn" onClick="delete_ask(); return false;">삭제</button></a></td>
